from RobotServer.keypoint_detection.predict_6_point import get_thy_keypoint
...
class KeypointDetectionThread(QThread):   #线程类
    ...
    def __init__(self, goal_force=2, save_path='', test_mode='1'):
        ...
        # === keypoint detection
        self.model_yolo = YOLO("/home/usai/IS-MAN/KeypointDetection/checkpoint/yolo11m_pose_best_thy_ketpoint.pt")  # load an official model
        ...
        #======= 相机在机器人基底坐标系下的变换 ============
        #------ 相机内参（ros驱动之后可以看到）
        camera_matrix = np.loadtxt("/home/usai/IS-MAN/SonoPilot/RobotServer/keypoint_detection/cameraMatrix.txt", delimiter=",")
        ...
        # R_wrist2cam = np.load('/home/usai/auto_RUSS/R_UI_Agent/keypoint_detection/R_wrist2cam_7x11.txt').astype(np.float32)
        # t_wrist2cam = np.load('/home/usai/auto_RUSS/R_UI_Agent/keypoint_detection/t_wrist2cam_7x11.txt').T.astype(np.float32)
        # self.T_wrist2cam = np.eye(4).astype(np.float32)
        # self.T_wrist2cam[:3,:3] = R_wrist2cam
        # self.T_wrist2cam[:3, 3] = t_wrist2cam  #*1000.0
        R_wc = np.loadtxt('/home/usai/IS-MAN/SonoPilot/RobotServer/keypoint_detection/R_wrist2cam_7x11.txt')
        t_wc = np.loadtxt('/home/usai/IS-MAN/SonoPilot/RobotServer/keypoint_detection/t_wrist2cam_7x11.txt')
        self.T_wrist2cam = np.eye(4)
        self.T_wrist2cam[:3, :3] = R_wc
        self.T_wrist2cam[:3, 3] = t_wc.flatten()
    ...
    def pre_view(self):
        ...
        if self.rgb_cv_img is not None:
            ...
            frame, self.select_point = get_thy_keypoint(model=self.model_yolo, rgb_img_original=self.frame_rgb_for_calculate, way=self.test_mode)
            self.need_caculate_normal = True
