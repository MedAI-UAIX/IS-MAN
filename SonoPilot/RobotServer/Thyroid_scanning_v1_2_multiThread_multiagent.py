...
from RobotServer.pub_recording_state import ContinuousPublisher, CheckPoint    #  是否采集扫查数据
...
try:
    ...
    rospy.logwarn(f"无法导入 RL 相关模块: {e}. 将仅使用规则兜底逻辑.")
    Agent = None
...
class UltrasoundServo:
    def __init__(self, client_cmd, goal_force=2, init_node=True, 
        ...
        if Agent is not None:
            self.weight_path = r'/home/usai/IS-MAN/SonoPilot/RobotServer/model.pkl'
            try:
                # ✅ 严格匹配 main_lr_test.py 中的初始化参数
                self.rl_agent = Agent(
                    action_dim=2,      # 测试代码中为 2 (left/right)
                    in_channels=1,     # 灰度图单通道
                    weight_path=self.weight_path
                )
                rospy.loginfo("✅ RL 模型加载成功 (action_dim=2, in_channels=1)")
    ...
    # ========== 修改：center_Callback 仅负责缓存数据 ==========
    def center_Callback(self, msg):
        # 最大面积，甲状腺起始x, w, 气管x, w, 图像有效区域起始/终止
        with self._area_lock:
            self._latest_area_data = list(msg.data)
    # =======================================================
    ...
    # ========== 新增：同步 RL/规则混合决策函数 ==========
    def _get_rl_action(self, pose):
        """
        同步获取 RL/规则混合决策动作（居中判断优先版）
        流程：
            1. 计算目标与中心的偏差 (deviation)
            2. 如果偏差 < 阈值: 直接返回 'center'
            3. 如果偏差 >= 阈值: 输入 RL 模型判断 'left' or 'right'
        :param pose: 'left', 'right', 'mid'
        :return: 'left', 'right', 'center' 或 None
        """
        # 1. 线程安全读取数据
        with self._area_lock:
            area_data = self._latest_area_data
        with self._img_lock:
            gray_img = self._latest_gray_img

        if area_data is None or len(area_data) < 7 or gray_img is None:
            return None

        thyroid_x, thyroid_w = int(area_data[0]), int(area_data[1])
        trachea_x, trachea_w = int(area_data[2]), int(area_data[3])
        img_start, img_end, area = int(area_data[4]), int(area_data[5]), area_data[6]

        # 2. 计算几何偏差 (判断是否居中的核心逻辑)
        img_center = (img_start + img_end) / 2.0
        
        # --- 根据 pose 计算目标中心 ---
        if pose == 'left':
            target_center = (trachea_x + trachea_w + thyroid_x + thyroid_w) / 2.0
        elif pose == 'right':
            target_center = (trachea_x + thyroid_x) / 2.0
        else:  # 'mid'
            target_center = (thyroid_x + thyroid_x + thyroid_w) / 2.0
        
        deviation = target_center - img_center
        threshold = 15.0 # 居中阈值，可根据实际效果微调

        # 3. 判断逻辑分支
        # --- 分支 A: 目标已在中心附近 ---
        if abs(deviation) < threshold:
            rospy.logdebug_throttle(1, f"[RL] 目标居中 (偏差: {deviation:.2f}), 动作: center")
            return 'center'

        # --- 分支 B: 目标不在中心，调用 RL 模型 ---
        # 图像预处理 (复刻 main_lr_test.py)
        h, w = gray_img.shape[:2]
        crop_x = max(0, min(thyroid_x - 20, w - 128))
        crop_y = max(0, min(h // 2 - 64, h - 128))
        cropped = gray_img[crop_y:crop_y+128, crop_x:crop_x+128]
        
        if cropped.shape != (128, 128):
            cropped = cv2.resize(cropped, (128, 128))
        
        # 转为 Tensor (1, 1, 128, 128)
        img_tensor = torch.from_numpy(cropped).float().unsqueeze(0).unsqueeze(0) / 255.0

        raw_action = 'center' # 默认安全动作
        if self.rl_agent is not None:
            try:
                # ✅ 调用 Agent.predict (仅传入图像，action_dim=2)
                action_idx = self.rl_agent.predict(img_tensor)
                # action_idx: 0 -> 'left', 1 -> 'right'
                raw_action = {0: 'left', 1: 'right'}.get(action_idx, 'center')
                rospy.logdebug_throttle(1, f"[RL] 推理结果: {raw_action} (idx={action_idx})")
            except Exception as e:
                rospy.logwarn_throttle(5, f'[RL] 推理异常，回退规则: {e}')
                # 推理失败时的规则兜底
                raw_action = 'right' if deviation > 0 else 'left'
        else:
            # 无模型时的纯规则逻辑
            raw_action = 'right' if deviation > 0 else 'left'

        # 4. ✅ 根据 pose 参数进行动作修正 (核心需求)
        # 例如: 在 'right' 模式下，模型认为要 'left'，实际应向右移
        if pose == 'right':
            if raw_action == 'left':
                return 'right'
            elif raw_action == 'right':
                return 'left'
        
        # 'left' 和 'mid' 模式下，动作方向与模型输出一致
        return raw_action
    # ===================================================
    ...
    def auto_center(self, pose):
        """
        同步 RL/规则混合居中控制
        pose: 'left', 'right', 'mid' —— 显式指定当前扫查部位
        """
        rospy.loginfo(f"[AutoCenter] 开始同步居中，当前部位模式: {pose}")

        # 等待第一帧图像数据到达 (设置超时)
        timeout_time = rospy.Time.now() + rospy.Duration(3.0)
        while self._latest_area_data is None and not self.stop_scanning_flag:
            if rospy.Time.now() > timeout_time:
                rospy.logwarn("[AutoCenter] 等待 unet_area 数据超时，跳过居中")
                return
            time.sleep(0.05)

        while not self.stop_scanning_flag:
            # ✅ 直接同步获取动作，无话题延迟
            adjustment = self._get_rl_action(pose)
            
            if adjustment is None:
                time.sleep(0.05)
                continue

            rospy.loginfo(f"[RL决策] 部位:{pose} | 指令: {adjustment}")
            ...
            if adjustment == 'left':
                ...
                rospy.loginfo(f"[RL决策] 部位 {pose} 已居中完成")
                break
            else:
                rospy.logwarn(f"[RL决策] 未知指令: {adjustment}")
