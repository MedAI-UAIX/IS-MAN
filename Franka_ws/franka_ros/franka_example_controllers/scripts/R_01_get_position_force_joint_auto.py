...
class MoveItFranka:
    def __init__(self) :
        ...
        self.joint_pose_publisher = rospy.Publisher('/joint_position_example_topic_controller/joint_angle_lmd', JointState, queue_size=10)
        ...
        self.joint_position_controller_name = 'joint_position_example_topic_controller'        
        ...
        if not self.cartesian_position_controller_name in current_controller_name:          
            ...
            print('加载位置控制器')
        if not self.joint_position_controller_name in current_controller_name:          
            _ = self.load_controller(self.joint_position_controller_name)  #预先加载位置控制器
            print('加载关节控制器')
        ...
        joint_controll_state = current_controller[self.joint_position_controller_name]
        print('位置控制器状态：{}'.format(position_controll_state))
        print('关节控制器状态：{}'.format(joint_controll_state))
        if force_controll_state == 'running':
            # 确保第一个控制器是位置控制
            state = self.switch_controllers([self.joint_position_controller_name], [self.hybrid_force_position_controller_name])
            print('初始化切换成 位置控制')
    ...
    def move_to_object_joint(self, pose, motion_time):
        """纯位置控制模式
        pose=[0.5, -0.5, 0.5, -1.0, 0.5, -0.5, 0.0], motion_time=5
            motion_time 为执行时间, 单位秒。此部分的运动插值直接由C++控制器完成，这里只需要发布最终的位置和时间即可
        """
        ...
        self.joint_pose_publisher.publish(joint_msg)
...
if  __name__ == "__main__":
    franka =  MoveItFranka()
    franka.move_to_object_joint(pose=[0.5, -0.5, 0.5, -1.0, 0.5, -0.5, 0.0], motion_time=5)
